...
def find_fair_price(tick, target_annual_growth = 15, margin_of_safety = 50):
    ...
    if equity_growth_YF < target_numbers['growth_rate'] or target_numbers['growth_rate'] < 0:
    ...
    return {
        'mos_price': mos_price, # not really necessary to send
        'fair_price': fair_price, # not really necessary to send
...
# print(find_fair_price(sys.argv[1], int(sys.argv[2]), int(sys.argv[3])))
